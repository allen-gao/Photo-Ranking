...
public class Canvas extends JPanel implements Observer {
		...
		if (model.isGrid()) {
			...
			for (int i = 0; i < imageObjects.size(); i++) {
				...
				try {
					...
					panel.setPreferredSize(new Dimension(this.getWidth(), panelHeight));
					JButton imageButton = new JButton(new ImageIcon(scaledImage));
					imageButton.addMouseListener(new MouseAdapter() {
						public void mouseClicked(MouseEvent e) {
							JFrame imageWindow = new JFrame(imageObject.getFile().getName());
	...
	public void resizePanels(int width, int height) {
		if (model.isGrid()) {
			...
			if (panelArray != null) {
				for (int i = 0; i < panelArray.size(); i++) {
					JPanel panel = panelArray.get(i);
					panel.setPreferredSize(new Dimension(width - 5, panelHeight));
				}
			
				this.setPreferredSize(new Dimension(width, panelArray.size() * (panelHeight + 5)));
				scrollPane.revalidate();
			}
		}
	}
	...
	public void update(Observable o, Object arg) {
		if (arg == "imageFiles" || arg == "stars" || arg == "grid") {
			loadImages();
			resizePanels(model.getFrame().getWidth(), model.getFrame().getHeight());
		}
	}
}
